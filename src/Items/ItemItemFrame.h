...
public:
	...
	) override
	{
		...
		{
			auto ItemFramePtr = cpp14::make_unique<cItemFrame>(a_BlockFace, a_BlockX, a_BlockY, a_BlockZ);
			auto ItemFrame = ItemFramePtr.get();
			if (!ItemFrame->Initialize(std::move(ItemFramePtr), *a_World))
			{
				return false;
			}
