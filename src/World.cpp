...
{
	auto PickupPtr = cpp14::make_unique<cPickup>(a_PosX, a_PosY, a_PosZ, a_Item, false, a_SpeedX, a_SpeedY, a_SpeedZ, a_LifetimeTicks, a_CanCombine);
	auto Pickup = PickupPtr.get();
	if (!Pickup->Initialize(std::move(PickupPtr), *this))
	{
...
UInt32 cWorld::SpawnExperienceOrb(double a_X, double a_Y, double a_Z, int a_Reward)
{
	...
	auto ExpOrbPtr = cpp14::make_unique<cExpOrb>(a_X, a_Y, a_Z, a_Reward);
	auto ExpOrb = ExpOrbPtr.get();
	if (!ExpOrb->Initialize(std::move(ExpOrbPtr), *this))
	{
		return cEntity::INVALID_ID;
	}
...
{
	auto BoatPtr = cpp14::make_unique<cBoat>(a_X, a_Y, a_Z, a_Material);
	auto Boat = BoatPtr.get();
	if (!Boat->Initialize(std::move(BoatPtr), *this))
	{
		return cEntity::INVALID_ID;
	}
...
{
	auto TNTPtr = cpp14::make_unique<cTNTEntity>(a_X, a_Y, a_Z, a_FuseTicks);
	auto TNT = TNTPtr.get();
	if (!TNT->Initialize(std::move(TNTPtr), *this))
	{
		return cEntity::INVALID_ID;
	}
...
{
	auto ProjectilePtr = cProjectileEntity::Create(a_Kind, a_Creator, a_PosX, a_PosY, a_PosZ, a_Item, a_Speed);
	if (ProjectilePtr == nullptr)
	{
		return cEntity::INVALID_ID;
	}

	auto Projectile = ProjectilePtr.get();
	if (!Projectile->Initialize(std::move(ProjectilePtr), *this))
	{
		return cEntity::INVALID_ID;
	}
